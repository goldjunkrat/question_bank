...
<script>
	...
	export default {
		...
		computed:{
			...mapState(['isFirstOne','isLastOne','finishQuestionNumber','totalQuestionNumber','questionOrder','baseUrl','userId','currentQuestionOrder','restQuestionNumber','wrongQuestionIndex','wrongQuestionIdList','wrongQuestionListLength','restWrongQuestionNumber','questionInfo'])
		},
		methods:{
			toNextQuestion(){
				// 题号+1
				...
				if(this.questionOrder>this.currentQuestionOrder){
					...
					// 发送请求获取下一题信息
					this.$store.dispatch("getFavoriteQuestionInfo",this.wrongQuestionIdList[this.wrongQuestionIndex+1])
